...
module.exports.register = function(name, dep) {
  if (_getDep(name) !== undefined) {
    var msg = 'Tried to register a dependency "' + name + '" that was already registered.';
    throw new Error(msg);
  }
  _setDep(name, dep);
};
 ...
 * });
 */
module.exports.override = function(overrides, callback) {
...
module.exports.call = function(cb) {
  ...
  if (rawArgs === '') {
    ...
    var args = rawArgs.split(',').map(trim).map(this.lookup);
    cb.apply(cb, args);
  }
};
