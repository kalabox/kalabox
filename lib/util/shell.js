...
var getGuiAdminWrapper = function(cmdString) {
  ...
  /*
   * Write applescript app to the Kalabox Downloads directory so the kbox binary
   * can execute it.
   */
  var asWriteApp = function() {
    // Get relevant paths
    var bin = path.join(__dirname, 'applescript');
    var sysRoot = core.deps.get('globalConfig').sysConfRoot;
    var downloadPath = path.join(sysRoot, 'downloads');
    var scripts = ['runAsApp.scpt', 'Kalabox.app'];

    scripts.forEach(function(value, key) {
      var source = path.join(bin, value);
      var dest = path.join(downloadPath, value);
      if (!fs.existsSync(dest)) {
        // @pirog: will copySync work?
  ...
  var asWrap = function(cmdString) {
    asWriteApp();
    var runAsApp = path.join(core.deps.get('globalConfig').sysConfRoot,
      'downloads', 'runAsApp.scpt');
    return [
      'osascript',
      runAsApp,
      '\'' + cmdString.replace(/\"/g, '\\"') + '\''
  ...
  switch (process.platform) {
    ...
    case 'darwin': return asWrap(cmdString);
  }
