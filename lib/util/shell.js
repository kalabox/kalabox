 ...
 * });
 */
var exec = exports.exec = function(cmd, opts, callback) {
  ...
  _shell.exec(cmd, _.extend(defaults, options), function(code, output) {
    var err = null;
    if (code !== 0) {
      err = new Error(
 ...
 * });
 */
var execAdminAsync = exports.execAdminAsync = function(cmd, options) {
  return execAsync(getAdminWrapper(cmd, options));
};
 ...
 * });
 */
var execAdmin = exports.execAdmin = function(cmd, callback) {
  exec(getAdminWrapper(cmd), callback);
};
...
exports.runCmdsAsync = function(cmds, options) {
  ...
  var cmd = cmds.join(' && ');
  return execAdminAsync(cmd, options);
};

exports.runCmds = function(cmds, callback) {
  // @todo: this is jank and not sure if works on windows?
  execAdmin(cmds, function(err, output) {
    callback(err, output);
  });
};
