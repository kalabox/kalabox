...
var execDarwin = function(cmd, callback) {
...
var execWin32 = function(cmd, callback) {
  // @todo: what happens before this exists?
  var opts = {
    silent: true,
    shell: '"C:\\Program Files (x86)\\Git\\bin\\sh.exe"'
  };
  _exec(cmd, opts, function(error, stdout, stderr) {
    if (error) {
...
var execLinux = function(cmd, callback) {
  var opts = {silent: true};
  _shell.exec(cmd, opts, function(code, output) {
    var err = null;
    if (code !== 0) {
...
exports.exec = function(cmd, callback) {
  ...
  if (process.platform === 'win32') {
    execWin32(cmd, callback);
  }
  else if (process.platform === 'darwin') {
    execDarwin(cmd, callback);
  }
  else {
    execLinux(cmd, callback);
  }
};

exports.execAdminAsync = function(cmd) {
  console.log(getAdminWrapper(cmd));
