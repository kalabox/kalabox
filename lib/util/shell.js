...
exports.spawn = function(cmd, opts) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    /*
     * When spawn needs a tty for stdin, replace pty place holder with a
     * pseudo terminal's stdin. This is needed for docker-compose run
     * commands that create a pty on the other side.
     */
    /*
     * The pty dep (npm module child_pty) should be injected by the GUI.
     */
    if (options.stdio[0] === 'pty') {
      // See if the pty dependency is registered.
      var pty = core.deps.get('pty', {optional: true});
      if (pty) {
        // If registered use the pty's stdin.
        options.stdio[0] = pty.stdin;
      } else {
        // Else use process.stdin.
        options.stdio[0] = process.stdin;
      }
    }
