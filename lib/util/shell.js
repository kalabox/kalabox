 ...
 * Build VERROR object
 */
function buildVerror(code, stdOut, stdErr) {
  return new VError({
    name: stdOut,
    info: {
      code: code,
...
exports.exec = function(cmd, opts) {
  ...
  // Logging functions.
  var execLog = core.log.make('SHELL EXEC');
  execLog.debug(format('Running command %s', cmd));

  // Promisify the exec
  return new Promise(function(resolve, reject) {
    _shell.exec(esc(cmd), options, function(code, stdout, stderr) {
      if (code !== 0) {
        reject(buildVerror(code, stdout, stderr));
      }
      else {
        execLog.debug(format('Running command %j', stdout));
        resolve(stdout);
      }
    });
  });
...
exports.spawn = function(cmd, opts) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var logName = ['SPAWN', path.basename(entrypoint).toUpperCase()].join(' ');
    var spawnLog = core.log.make(logName);
    spawnLog.debug(format(
      'Using %s to run %s in %s mode with environment %j detached %s',
      entrypoint,
      cmd,
      options.mode,
      options.env,
    ...
    if (options.stdio === 'pipe' || options.stdio[1] === 'pipe') {
      run.stdout.on('data', function(buffer) {
        spawnLog.debug(format('Collected output: %s', _.trim(String(buffer))));
    ...
    run.on('error', function(buffer) {
      spawnLog.debug(format('Collected error: %s', _.trim(String(buffer))));
      stdErr = stdErr + String(buffer);
    });

    // Callback when done
    run.on('close', function(code) {
      spawnLog.debug(format('Spawn exited with code: %s', code));
      if (code !== 0) {
        reject(buildVerror(code, stdOut, stdErr));
      }
      else {
