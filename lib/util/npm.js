...
var npmInstance = _.once(function() {
  return Promise.promisifyAll(require('npm'));
});
...
var isNpmPackage = function(id) {
  ...
  // Init NPM.
  return npmInstance().loadAsync()
  // Silently query for info on package.
  .then(function() {
    var silent = true;
    return npmInstance().commands.viewAsync([id], silent);
  })
