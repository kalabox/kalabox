...
exports.updateBackends = function(callback) {
  ...
  var backends = [
    deps.lookup('config').engine.split('@'),
    deps.lookup('config').services.split('@')
  ];
  backends.each(function(backend) {
    var backendName = backend[0];
    if (fs.existsSync(path.join(src, 'node_modules', backendName, '.git'))) {
      console.log('It looks like you installed ' + backendName + ' with git.' +
        ' Update by running a git pull on master in there.');
    }
    else {
      installbackends.push(backend.join('@'));
    }
  });
  nodeOp('install', installbackends, callback);
};
