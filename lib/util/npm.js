...
// @todo right now this doesnt quite work the way we want it to
// and can probably be removed but going to keep it around for awhile longer
exports.updateKalaboxCore = function(callback) {
  npm.load(function(err) {
    if (err) {
      callback(err);
    }
      ...
      if (fs.existsSync(path.join(src, '.git'))) {
        console.log('It looks like you installed kbox with git.' +
          ' Update by running a git pull and then kbox update again.');
        callback();
      }
      else if (npm.globalDir.indexOf(src) > -1) {
        console.log('It looks like you didn\'t npm install kalabox globally.' +
          ' Update your kbox directory manually.');
        callback();
      }
      else {
        nodeOp(
