...
/*
 * Return the correct color for the kind of message
 */
var getPrefixColor = function(kind) {
  switch (kind) {
...
var logger = _.once(function() {
  ...
  return Promise.try(function() {
    ...
    .then(function() {
      // Gte log level for the console. Always debug log to teh gui so we can
      // see WTFIGO
      var logLevelConsole = globalConfig.logLevelConsole || 'status';
      var argv = kbox.tasks.partitionArgv(process.argv.slice(2));
      if (argv.options.debug || mode === 'gui') {
        logLevelConsole = 'debug';
      } else if (argv.options.verbose || mode === 'gui') {
        logLevelConsole = 'verbose';
      }
      // Create logger.
      return new winston.Logger({
        transports: [
          ...
          new winston.transports.File({
            ...
            level: globalConfig.logLevel || 'verbose'
          })
        ]
      });
    });
  ...
  _.each(actionNames, function(name) {
    api[name] = function() {
      ...
      if (opts.prefix && typeof args[0] === 'string') {
        args[0] = [
          chalk[getPrefixColor(name)](opts.prefix),
          args[0]
        ].join(chalk.grey(' ==> '));
      }
