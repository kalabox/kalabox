...
var expand = function(imageName) {
  ...
  // Get docker registry user from global config.
  var dockerRegistryUser = core.deps.get('globalConfig').engineRepo;
  ...
  if (!o.tag && o.user === dockerRegistryUser) {
    var profile = null;
    try {
      profile = core.deps.get('globalConfig').profile;
    } catch (err) {
      profile = null;
    }
    // @bcauldwell: Change before moving
    var currentVersion = profile === 'dev-v0.10.0' ? '0.9.0' : version.get();
    o.tag = 'v' + version.truncatePatch(currentVersion);
  }
