...
var expand = function(imageName) {
  ...
  var devMode = globalConfig.devMode;
  ...
  if (!o.tag && o.user === dockerRegistryUser) {
    // If devMode is on pull the most up to date image, if not grab the
    // image for the minor version
    var minVer = version.truncatePatch(version.get());
    o.tag = (devMode === true) ? 'dev' : 'v' + minVer;
  }
