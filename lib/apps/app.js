...
var App = function(name, config)   {
  ...
  self.dataContainerName = null;
};

App.prototype.setupComponents = function() {
  var self = this;
  self.components = {};
  // Ensure the cids dir has been created.
  if (!fs.existsSync(self.config.appCidsRoot)) {
    fs.mkdirSync(self.config.appCidsRoot);
  }
  for (var key in this.config.appComponents) {
    ...
    if (key === 'data') {
      self.dataContainerName = ['kb', self.name, 'data'].join('_');
    }
  }
};
