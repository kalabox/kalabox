...
var appAlreadyExists = function(app, callback) {
  var exists = false;
  core.deps.call(function(docker) {
    docker.listContainers({all: 1}, function(err, containers) {
      if (err || !containers) {
        callback(err, exists);
      } else {
...
var createStartupServices = function(copts, done) {
  vasync.forEachPipeline({
    'func': ctn.create,
    'inputs': _.toArray(copts)
...
/**
 * Small helper function for vasync to pass in multiargs
 */
var _startupServiceStart = function(service, callback) {
  ctn.start(service.cid, service.startOpts, function(err, data) {
    callback(err, data);
  });
};
  ...
  appAlreadyExists(app, function(err, alreadyExists) {
    if (err) {
      ...
      core.deps.call(function(events) {
        events.emit('pre-init', app, function() {
          var components = _.toArray(_.cloneDeep(app.components));
          vasync.forEachPipeline({
            'func': cmp.create,
            'inputs': components
          }, function(err, results) {
            events.emit('post-init', app, function() {
              if (done) {
                done(err);
              }
            });
    ...
    async.series([
      ...
      function(next) {
        if (!_.isEmpty(servicesNotCreated)) {
          var copts = _.pluck(servicesNotCreated, 'createOpts');
          createStartupServices(copts, function(err, results) {
            if (err) {
              throw err;
            }
            _.forEach(results.successes, function(container) {
              services[container.name].cid = container.cid;
            });
            next(null);
          });
        }
...
var kill = function(app, done) {
  core.deps.call(function(events) {
    events.emit('pre-kill', app, function() {
      var components = _.toArray(_.cloneDeep(app.components));
      vasync.forEachPipeline({
        'func': cmp.kill,
        'inputs': components
      }, function(err, container) {
        events.emit('post-kill', app, function() {
          if (done) {
            done(err);
          }
        });
...
var remove = function(app, done) {
  core.deps.call(function(events) {
    events.emit('pre-remove', app, function() {
      var components = _.toArray(_.cloneDeep(app.components));
      vasync.forEachPipeline({
        'func': cmp.remove,
        'inputs': components
      }, function(err, results) {
        events.emit('post-remove', app, function() {
          if (done) {
            done(err);
          }
        });
...
var pull = function(app, done) {
  core.deps.call(function(events) {
    events.emit('pre-pull', app, function() {
      var components = _.filter(app.components, function(component) {
        return !component.image.build;
      });
      ...
      vasync.forEachPipeline({
        ...
        events.emit('post-pull', app, function() {
          if (done) {
            done(err);
          }
        });
...
var build = function(app, done) {
  core.deps.call(function(events) {
    events.emit('pre-build', app, function() {
      var components = _.filter(app.components, function(component) {
        return component.image.build === true;
      });
      var images = _.pluck(components, 'image');
      vasync.forEachPipeline({
        'func': image.build,
        ...
        events.emit('post-build', app, function() {
          if (done) {
            done(err);
          }
        });
