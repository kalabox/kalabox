  ...
  docker.listContainers(function (err, containers) {
    containers.forEach(function (containerInfo) {
      ...
      if (name != null && ctn.name.isUserDefined(name)) {
        apps[name.app_name].status = 'enabled';
      }
    });
    ...
    docker.listContainers({all: 1}, function (err, containers) {
      containers.forEach(function (containerInfo) {
        var name = ctn.name.parse(containerInfo.Names[0]);
        if (name != null && ctn.name.isUserDefined(name) && !apps[name.app_name].status) {
          apps[name.app_name] .status = 'disabled';
        }
      });
 ...
 *
 * @param obj app or manager object
...
var purgeContainers = function(callback) {
  docker.listContainers({all: 1}, function (err, containers) {
    containers.forEach(function (containerInfo) {
      var name = ctn.name.parse(containerInfo.Names[0]),
      shouldPurge = name === null || !ctn.name.isKalaboxName(name);
      if (shouldPurge) {
