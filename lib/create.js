...
module.exports = function() {
  ...
  var buildKboxJson = function(app, result, callback) {
    ...
    var kalaboxJson = require(sourcePkgFile);
    app.options.forEach(function(opt) {
      if (opt.conf) {
    ...
    var destPkgFile = path.join(
      deps.lookup('config').appsRoot,
      result.name,
      'kalabox.json'
    );
    fs.writeFileSync(destPkgFile, JSON.stringify(kalaboxJson, null, 2));
