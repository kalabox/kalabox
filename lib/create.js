    ...
    fs.copy(source, dest, function(err) {
      if (err) {
        ...
        instantiateApp(app, result, options, function(err) {
          ...
          if (install) {
            appEngine.get(result.name, function(err, loadedApp) {
              if (err) {
                return done(err);
              }

              deps.register('appConfig', app.config, function(err) {
                if (err) {
                  return done(err);
                }

                deps.register('app', app, function(err) {
                  if (err) {
                    return done(err);
                  }
                  ...
                  appEngine.loadPlugins(loadedApp, function(err) {
                    ...
                    appEngine.install(app, function(err) {
                      if (err) {
                        return done(err);
                      }
                      ...
                      if (start) {
                        appEngine.start(app, function(err) {
                          if (err) {
                            done(err);
                          }
                          else {
