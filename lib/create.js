...
module.exports = function() {
  ...
  var createApp = function(app, results, callback) {
    ...
    // @todo: how do we know which app to grab?
    // get package version from global config for non-dev
    // use master for dev-mode
    .then(function() {

      // Get the stuff we need for downloading
      var downloadDir = util.disk.getTempDir();
      var downloadFiles = util.download.downloadandExtractFiles;
      ...
      // @todo: will this work on windows? do we need ZIP action?
      var tarUrl = {
        protocol: 'https:',
        host: 'github.com',
        pathname: ['kalabox', project, 'tarball', version].join('/')
      };

      return Promise.fromNode(function(callback) {
        downloadFiles([url.format(tarUrl)], downloadDir, callback);
      })
      ...
      .then(function(dir) {
        var source = path.join(downloadDir, dir, 'app');
        return Promise.fromNode(function(cb) {
