...
module.exports = function() {
  ...
  /*
   * Download or the copy the app skeleton or copy over the dir specified in FROM
   */
  var createAppSkeleton = function(app, dest, from) {

    // Assess our situation
    var localPath = from || app.task.pkg.dir;
    var needsDownload = (app.task.pkg.url && !localPath) ? true : false;
    log.debug(format('App requires download: %s', needsDownload));
    ...
    return Promise.try(function() {
      ...
      // Otherwise just copy from --from
        ...
        return Promise.fromNode(function(cb) {
          ...
          // Do the copying
          fs.copy(localPath, dest, options, cb);
          log.debug(format('Copied %s to %s', localPath, dest));
  ...
  var createApp = function(protoApp, results, callback) {
    ...
    var isAppByFiles = function() {
      var kalaboxYaml = path.join(dest, 'kalabox.yml');
    ...
    if (kbox.core.deps.contains('app') || isAppByFiles()) {
      throw new Error('Looks like an app already exists at ' + dest);
    }

    // Emit pre create event.
    kbox.core.log.status('Creating.');
    return kbox.core.events.emit('pre-create', results)
    ...
    .then(function() {
      ...
      .tap(function(app) {
        ...
        // Set a flag so app.start events know if we are creating or not
        app.creating = true;
