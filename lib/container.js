...
exports.name = {
  ...
  createUserDefined: function (app_name, component_name) {
    return util.name.create([this._USER_DEFINED_PREFIX, app_name, component_name]);
  },
  createBuiltIn: function (app_name, component_name) {
    return util.name.create([this._BUILT_IN_PREFIX, app_name, component_name]);
  },
  parse: function (name) {
    // remove white space from front of container name
    var _name = name[0] === ' ' ? name.substring(1) : name;
