 ...
 * this in the future
 */
exports.createGen = function(copts, callback) {
  console.log(copts);
  docker.createContainer(copts, function(err, container) {
    if (err) {
      throw err;
    }
    if (container) {
...
/**
 * Starts a generic container with given ops
 */
exports.startGen = function(container, callback) {
  var sopts = container.startOpts;
  docker.getContainer(container.cid).start(sopts, function(err, data) {
    callback(err, data);
  });
};
