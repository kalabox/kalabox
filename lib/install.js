...
exports.run = function(opts, callback) {
  ...
  if (isCli) {
    ...
    installFramework.events.on('pre-step', function(ctx) {
      // Logging stuff
      var perc = ctx.state.stepIndex === 0 ?
        0 :
        ((ctx.state.stepIndex) / ctx.state.stepCount) * 100;
      kbox.status.update('[ %s%% ] - %s', _.round(perc, 0), ctx.step.description);
  ...
  if (isCli) {
    ...
    installFramework.events.on('end', function(ctx) {
      if (ctx.state.status) {
        ...
        console.log(chalk.green('Huzzah! Victory!'));
      }
    });
  }
  ...
  // Print extra stuff in cli.
  .catch(function(err) {
    if (isCli) {
      console.log(
        chalk.cyan('--') +
        chalk.red('INSTALL FAILURE.') +
        chalk.cyan('--')
      );
    }
    throw err;
