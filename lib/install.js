...
var B2D_URL_V1_3_0 = 'https://github.com/boot2docker/osx-installer/releases/download/v1.3.3/Boot2Docker-1.3.3.pkg';
        ...
        async.series([
          ...
          function(next) {
            if (!b2dIsInstalled) {
              disk.getMacVolume(function(err, volume) {
                ...
                adminCmds.unshift(installer.buildInstallCmd(pkg, volume));
                next(null);
              });
            }
            else {
          ...
          function(next) {
            if (!dnsIsSet) {
              ...
              adminCmds.unshift(installer.buildDnsCmd(KALABOX_DNS_FILE));
            }
            next(null);
          },
          ...
          function(next) {
            if (adminCmds) {
              var child = installer.runCmdsAsync(adminCmds);
