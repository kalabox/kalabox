/**
 * Module for interfacing with the Kalabox container system implementation, and the provider of the container system.
 * NOTE: The container system and the provider are not always the same. For example using docker on a Mac would use docker as the container system, however it would be provided by boot2docker.
 * @module kbox.engine
 */
...
var makeProviderError = function(msg) {
  return new Error(['Provider "' + engine.getProviderName() + '"', msg].join(' '));
};
