...
var makeProviderError = function(msg) {
  ...
  var msg = _.filter(msgParts).join(' ');
...
// @todo: document
var query = function(image, cmd, createOpts, startOpts, callback) {
  verifyProviderIsReady(function(err) {
    if (err) {
      callback(err);
    } else {
      engine.query(image, cmd, createOpts, startOpts, function(err, stream) {
        callback(err, stream);
      }, function() {
 ...
 * @arg {string} image - Name of image to run query in
 * @arg {string} cmd - Command (query) to run
 * @arg {object} startOptions [optional] - Object containing the start options for the container.
 * @arg {object} createOptions [optional] - Object containing the create options for the container.
 * @arg {function} callback - Callback called when the container has been started.
 * @arg {error} callback.error
 ...
 * });
 */
exports.queryString = function(image, cmd, createOpts, startOpts, callback) {
  query(image, cmd, createOpts, startOpts, function(err, stream) {
    if (err) {
      callback(err);
    } else {
      var buff = null;
