...
exports.run = function(image, cmd, createOptions, startOptions, callback) {
  verifyProviderIsReady(function(err) {
    if (err) {
      ...
      engine.run(image, cmd, process.stdin, process.stdout, createOptions, startOptions, callback);
    }
  });
};

// @todo: document
var query = exports.query = function(image, cmd, createOptions, startOptions, callback) {
  verifyProviderIsReady(function(err) {
    if (err) {
      callback(err);
    } else {
      engine.query(image, cmd, createOptions, startOptions, function(err, stream) {
        callback(err, stream);
      }, function() {
...
// @todo: document
exports.queryString = function(image, cmd, createOptions, startOptions, callback) {
  query(image, cmd, createOptions, startOptions, function(err, stream) {
    if (err) {
      callback(err);
    } else {
      var buff = null;
...
/*var run = function(image, cmd, streamo, createOptions, startOptions, callback) {
  verifyProviderIsReady(function(err) {
    if (err) {
      ...
      engine.run(image, cmd, streamo, createOptions, startOptions, callback);
    }
  });
};
exports.run = run;*/
