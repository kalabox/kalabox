...
var isInstalled = function(callback) {
  ...
  var whichWhere = (process.platform === 'win32') ? 'where' : 'which';
  var cmd = [whichWhere, B2D_EXECUTABLE].join(' ');
...
var prepareInstall = function(callback) {
  parseProfile(function(profile) {
    var iso = path.resolve(profile.ISO.replace(/"/g, ''));
...
exports.init = function(intf) {
  ...
  intf.prepareInstall = prepareInstall;
