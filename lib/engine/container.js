// @todo: THIS ONLY EXISTS FOR TESTING PURPOSES. AS SOON AS THERE ARE OTHER
// TESTS FOR THIS WE SHOULD DELETE THIS
...
var getContainer = function(cid) {
  return core.deps.call(function(docker) {
...
exports.create = function(copts, callback) {
  var args = [
    ...
    function(err, data) {
      ...
      var container = {};
      if (data) {
        // @todo: we need this to start non-app containers until we have
        // CID-files for startup services
        if (copts.name) {
          container = {
            cid: data.id,
            name: copts.name
...
exports.name = {
  ...
  createUserDefined: function(appName, componentName) {
    return util.name.create(
      [this._USER_DEFINED_PREFIX, appName, componentName]
    );
  },
