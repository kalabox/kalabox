...
var add = exports.step = function(handler) {
  var step = _.cloneDeep(intf);
  // If handler doesn't include a callback, treat it as sync.
  if (handler.length === 0) {
    handler.apply(step, []);
...
add(function() {
  this.name = 'foo';
  this.deps = [];
  this.all.darwin = function(state) {
    state.foo = 'bar';
  };
});
...
add(function() {
  ...
  this.all.win32 = function(state, next) {
    state.downloader.download('http://howtowin.edu/how2.pdf', function() {
      next();
    });
  };
});
