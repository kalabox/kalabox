...
var log = require('./core/log.js').make('REQUIRE');
...
var requireFromOtherDirs = function(kbox, id, app, callback) {
  ...
  var dirs = [
    ...
    globalConfig.sysPluginRoot,
    globalConfig.userPluginRoot
  ];
  if (app) {
    ...
    if (typeof mod === 'object') {
      ...
      if (mod.length === 0) {
        ...
        if (args.length === 1 && args[0] === 'kbox') {
          ...
          // Decorate any errors.
          .catch(function(err) {
            // Print a hopefully helpful warning for certain kinds of errors
            // @todo: this is pretty presumptuous and might be bad
            if (_.includes(err.message, 'Cannot read property')) {
              var warning = [
                'OH NO!',
                'It looks like something is wrong with your %s app named %s.',
                'We recommended purging this app and reinstalling it.',
                'This should help get it back into the correct state.'
              ];
              log.warn(format(warning.join(' '), id, app.name));
            }
