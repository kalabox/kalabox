 ...
 * Constants.
 */
var IMAGE_NAME = 'kalabox/syncthing:stable';
...
exports.isRunning = function() {
  // Inspect.
  return kbox.engine.inspect(CONTAINER_NAME)
...
var remove = exports.remove = function(counter) {
  ...
  // Remove container.
  return kbox.engine.remove(CONTAINER_NAME)
  ...
  .then(function(isInstalled) {
    if (isInstalled) {
      if (counter > 0) {
        ...
        throw new Error('Count not remove container ' + CONTAINER_NAME);
      }
    }
  });
...
exports.start = function(opts) {
  // Start container.
  return kbox.engine.start(CONTAINER_NAME);
};

// Stop syncthing container.
exports.stop = function() {
  // Stop container.
  return kbox.engine.stop(CONTAINER_NAME);
};
