...
var logInfo = function(msg, json) {
  if (!json) {
    console.log(msg);
  } else {
    console.log(msg + ' ===> ' + JSON.stringify(json, null, '  '));
  }
};
...
Sync.prototype.setConfig = function(config) {
  var self = this;
  logInfo('SYNC => SET CONFIG ' + self.address, config);
  return rest.configPost(this.address, config)
  .then(function() {
    return self.getConfig()
    .then(function(configOut) {
      logInfo('SYNC => SET CONFIG CHECK ' + self.address, config);
    });
  });
};
...
Sync.prototype.ping = function() {
  logInfo('SYNC => PING ' + this.address);
...
Sync.prototype.removeFolder = function(id) {
  ...
  return self.getConfig()
    .then(function(config) {
      logInfo('SYNC => Removing folder ' + id + ' ' + self.address);
...
Sync.prototype.addFolder = function(id, path, remoteSync, overrideOptions) {
  var self = this;
  logInfo('SYNC => Adding folder ' + id + ' ' + self.address);
  var options = {
    ReadOnly: false,
    RescanIntervalS: 5,
    IgnorePerms: true,
  ...
  return Promise.join(
    ...
    .then(function() {
      return logInfo('SYNC => Added folder ' + id + ' ' + self.address);
    });
};
...
Sync.prototype.restart = function() {
  var self = this;
  logInfo('SYNC => attemping restart: ' + self.address);
  return rest.restart(self.address)
    .then(function() {
      logInfo('SYNC => restarting: ' + self.address);
    });
};

Sync.prototype.restartWait = function() {
  var self = this;
  logInfo('SYNC => waiting for restart to finish: ' + self.address);
  return self.restart()
  .then(function() {
    return self.wait()
    .then(function() {
      console.log('SYNC => done restarting: ' + self.address);
    });
  });
};
...
Sync.prototype.shutdown = function() {
  var self = this;
  logInfo('SYNC => attempting shutdown: ' + self.address);
  return rest.shutdown(this.address)
  .then(function() {
    logInfo('SYNC => shutdown: ' + self.address);
  });
};
