 ...
 */
function Task(description, cmd, timeoutDuration) {
  this._description = description;
  if (typeof cmd !== 'function') {
    var msg = 'Command passed to task [' + cmd + '] is not a function.';
    var err = this._wrapError(new Error(msg));
    throw err;
  }
  this._cmd = cmd;
  this._timeoutDuration = timeoutDuration || DEFAULT_TIMEOUT;
}
 ...
 * var task = new Task('mytask', cmd, timeout);
 *
...
Task.prototype.run = function(callback) {
  ...
  var timeout = setTimeout(function() {
    ...
    if (cbTimeout) cbTimeout(self._wrapError(timeoutErr))
  ...
  this._cmd(function(err) {
    ...
    // Call the callbacks.
    if (err) {
      if (cbError) cbError(self._wrapError(err));
    } else {
      if (cbEnd) cbEnd(null);
    }
  });
