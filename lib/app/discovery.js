...
var getAppRegistry = function() {
  ...
  return fs.readFileAsync(globalConfig.appRegistry, {encoding: 'utf8'})
...
var getDirsAppRegistry = exports.getDirsAppRegistry = function() {
  // Get app registry.
...
var inspectDir = function(dir) {
  ...
  .then(function() {
    return util.yaml.toJson(filepath);
  })
  // Handle no entry error.
  .catch(function(err) {
    if (err.code === 'ENOENT') {
...
exports.getAppDir = function(appName, callback) {
...
var isLocked = function(filepath) {
  // Read file.
  return fs.readFileAsync(toLockfilepath(filepath), {encoding: 'utf8'})
...
var actionAppDir = function(dir, action) {
  var filepath = getGlobalConfig().appRegistry;
  ...
  .then(function() {
    ...
    .then(function(appDirs) {
      ...
      // Do the correct thing based on the action
      ...
      if (action === 'add' && !alreadyExists) {
        ...
        appDirs.splice(appDirs.indexOf(dir), 1);
        return setAppRegistry(appDirs);
      }
...
/*
 * Add a directory to the app registry
 */
exports.registerAppDir = function(dir, callback) {
  // Add the directory
  return actionAppDir(dir, 'add')
  // Return.
  .nodeify(callback);
};

/*
 * Remove a directory from the app registry
 */
exports.removeAppDir = function(dir, callback) {
  // Remove the directory
  return actionAppDir(dir, 'remove')
  // Return.
  .nodeify(callback);
};
