 ...
 * });
 */
var emit = exports.emit = function(name, state, cb) {
...
/*
 * Incremental counter for giving event context a unique id.
 */
var eventContextCounter = 0;
...
function EventContext(id) {
  if (id === undefined) {
    // Get unique id and then increment.
    this.id = eventContextCounter;
    eventContextCounter += 1;
  } else {
    this.id = id;
  }
}
