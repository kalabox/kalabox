...
EventEngine.prototype.emit = function(name, state, done) {
  ...
  // Get a hint of where the event is coming from
  // @todo: improve this to include more properties as appropriate
  var hint = (state && state.path) ? state.path : 'unknown';
  log.debug(format('Emitting event [%s].', name), hint);
