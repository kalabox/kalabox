...
var register = exports.register = function(name, dep, done) {
  ...
  _setDep(name, dep);
  if (name === 'app') {
    events.emit('dep-registered', {key:name, val:dep}, function(err) {
      if (err) {
        done(err);
      } else {
        events.emit('app-registered', dep, done);
      }
    });
  } else {
    done();
...
var inspect = exports.inspect = function(cb) {
