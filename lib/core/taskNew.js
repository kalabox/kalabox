...
var invariant = function(task) {
  ...
  // Validate allowArgv.
  if (typeof task.allowArgv !== 'boolean') {
    throw new TypeError('Invalid task allowArgv: ' + pp(task));
  }
...
var decorate = function(task) {
  var helpOption = _.find(task.options, function(option) {
    return option[1] === 'khelp';
  });

  if (!helpOption) {
    task.options.push(['', 'khelp', 'Display usage and options.']);
  }
};
...
var getHelpMessage = function(task) {
  return 'Usage: ' + getName(task) + ' [OPTION]\n' +
    '\n' +
    '[[OPTIONS]]\n';
};
...
var run = exports.run = function(task, callback) {
  ...
  if (argv.options.khelp) {
    parser.setHelp(getHelpMessage(task));
  ...
  // Enforce task.allowArgv.
  if (!task.allowArgv && context.argv.length > 0) {
    var helpMessage = '\nInvalid argument: ' + context.argv[0] + '\n\n' +
      getHelpMessage(task);
