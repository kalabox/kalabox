...
var getName = function(obj) {
  if (isBranch(obj)) {
    ...
    if (obj.path.length < 1) assert(false);
    return obj.path[0];
  } else {
    assert(false);
  }
...
var compareBranchOrTask = function(x, y) {
  ...
  if (typeof xName !== 'string' || xName.length < 1) assert(false);
  if (typeof yName !== 'string' || yName.length < 1) assert(false);
...
var addChild = function(branch, child) {
  // Validate.
  if (!isBranch(branch)) assert(false);
  if (!isBranchOrTask(child)) assert(false);

  // Branch already contains a child with the same name.
  if (branchContainsChild(branch, child)) assert(false);
...
var addTask = function(task) {
  ...
  var rec = function(node, path) {
    ...
    // Validate.
    if (!isBranch(node)) assert(false);
    if (path.length === 0) assert(false);
    ...
    if (cursor && isTask(cursor)) assert(false);
