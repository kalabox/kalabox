...
var run = exports.run = function(task, callback) {
  ...
  if (kind.kind === 'argv') {
    ...
    // Find the index of the splitter '--'.
    var indexOfSplitter = _.reduce(task.argv, function(acc, arg, index) {
      return arg === '--' ? index : acc;
    }, undefined);

    if (indexOfSplitter) {

      // Only parse what comes after the splitter.
      // Set argv to what comes before the splitter.
      argv = task.argv.slice(0, indexOfSplitter);
      // Grab and parse what comes after the splitter.
      var afterSplit = task.argv.slice(indexOfSplitter + 1, task.argv.length);
      ...
      // No splitter, so no parsing needed.
      argv = task.argv;
...
// @todo: remove, only here to show example delegate task.
registerTask(function(task) {
  task.path = ['foo', 'bar'];
  task.description = 'Toggle shield generator operation.';
  task.kind = 'delegate';
  task.func = function() {
