...
var DEFAULT_GLOBAL_CONFIG = {
  domain: 'kbox',
  engine: 'kalabox-engine-docker@0.10.0',
  engineRepo: 'kalabox',
  codeDir: 'code',
  services: 'kalabox-services-kalabox@0.10.0',
  apps: [
    'kalabox-app-pantheon@0.10.0'
  ],
  profile: 'standard',
  sharing: true,
  stats: {
...
exports.getEnvConfig = function() {
  ...
  // Grab the kbox version
  var packageJson = getPackageJson();
  var kboxVersion = packageJson.version;

  // Grab the provision file path
  var provisionedFile = path.join(env.getSysConfRoot(), 'provisioned');

  // Grab the installed file
  var iFile = path.join(env.getSysConfRoot(), 'installed.json');
  var cI = (fs.existsSync(iFile)) ? require(iFile) : {KALABOX_VERSION:'0'};
