...
var getB2dProfile = function() {
  return deps.lookup('config').sysConfRoot + '/b2d.profile';
};
module.exports.getB2dProfile = getB2dProfile;

var hasProfile = function(callback) {
  var B2D_PROFILE = getB2dProfile();
  if (fs.existsSync(B2D_PROFILE)) {
    kenv.setB2dProf(B2D_PROFILE);
    callback(null, true);
  } else {
    var err = new Error('You need a Boot2Docker profile. Try kbox install!');
    callback(err, false);
  }
};
...
function verifyB2dProfile(callback) {
  hasProfile(function(err, hasProfile) {
    if (err) {
      throw err;
    } else {
      callback();
 ...
 * });
 */
var down = function(b2d, attempts, callback) {
 ...
 * });
 */
var up = function(b2d, attempts, callback) {
