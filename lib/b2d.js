...
var hasProfile = function(callback) {
  var B2D_PROFILE = path.join(core.deps.lookup('config').sysConfRoot, 'b2d.profile');
  if (fs.existsSync(B2D_PROFILE)) {
    core.env.setB2dProf(B2D_PROFILE);
    callback(true);
  } else {
    callback(false);
  }
...
var _down = function(attempts, callback) {
  execB2dCommand('down', function(err, output) {
    if (err && attempts === 0) {
      throw err;
    }
    else if (err && attempts > 0) {
      _down(--attempts, callback);
    }
    else {
      events.emit('post-down', null, function() {
        callback();
      });
    }
  });
...
exports.down = function(attempts, callback) {
  events.emit('pre-down', null, function() {
    _down(attempts, callback);
  });
};

var _up = function(attempts, callback) {
  execB2dCommand('up', function(err, output) {
    if (err && attempts === 0) {
      throw err;
    }
    else if (err && attempts > 0) {
      _up(--attempts, callback);
    }
    else {
      events.emit('post-up', null, function() {
        callback();
      });
    }
  });
...
exports.up = function(attempts, callback) {
  events.emit('pre-up', null, function() {
    _up(attempts, callback);
  });
};
 ...
 * });
 */
var init = function(attempts, callback) {
