...
var cleanup = exports.cleanup = function(callback) {
  ...
  .each(function(appName) {
    ...
    .each(function(component) {
      ...
      core.log.info('WARNING! ORPHANED CONTAINER DETECTED => REMOVING...');

      // Check if our container actually exists, we might have a stale cidfile
      return engine.containerExists(component.containerId)

      // If it exists remove the container, if it doesnt delete the stale cidfile
    ...
    // Remove the orphaned app from the app registry
    .then(function() {
      console.log(appName);
    });
  })
