 ...
 */
var refreshComponents = function(app) {
  ...
  .then(function(isUp) {
    if (isUp) {
      ...
      return Promise.map(cmps, function(cmp) {
        return engine.inspect({files: app.composeCore, service: cmp.name})
        .then(function(data) {
          if (!_.isEmpty(data)) {
            cmp.cid = data.Id;
...
var create = exports.create = function(name, config, callback) {
  ...
  .then(function(provider) {
    ...
    // Set this in the env
    core.env.setEnvFromObj(app, 'app');
...
var start = exports.start = function(app, callback) {
  ...
  // Our stuff shoud have IDs now so lets get them
  .then(function() {
    return refreshComponents(app);
  })
