 ...
 */
var loadConfig = function(obj) {
  obj.tasks = {};

  // TODO: Validate config location
  var configFile = path.resolve(obj.path, '.kalabox.json');
  if (!fs.existsSync(configFile)) {
    throw  new Error("File does not exist: " + configFile);
  }
  // Load config
  obj.config = require(configFile);
  ...
  obj.appdomain = obj.name + '.' + obj.kconfig.domain;
  obj.dataPath = path.resolve(obj.kconfig.appDataPath, obj.name);
  obj.cidPath = path.resolve(obj.dataPath, 'cids');
  obj.url = 'http://' + obj.appdomain;
  obj.prefix = obj.name + '_';

  // Validate components exist
  if (!obj.config.components) {
    throw new Error('App config must contain at least 1 component.');
  }
  // Set data component for special handling
  obj.hasData = obj.config.components.hasOwnProperty('data');
  obj.dataCname = obj.hasData ? 'kb_' + obj.prefix + 'data' : null;
};
...
var loadPlugins = function(app) {
  ...
  for (var x in default_plugins) {
    if (!app.config.plugins[default_plugins[x]]) {
      app.config.plugins[default_plugins[x]] = {};
    }
  }
  _.map(app.config.plugins, function(plugin, key) {
    plugin.key = key;
    ...
    if (src !== false) {
      app.plugins[key] = require(src)(plugin, app.manager, app);
    }
  });
};
