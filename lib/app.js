...
// @todo: bcauldwell - Engine should support promises.
var engine = Promise.promisifyAll(require('./engine.js'));
// @todo: bcauldwell - Services should support promises.
var services = require('./services.js');
var servicesVerifyAsync = Promise.promisify(services.verify);
...
var createComponent = function(app, name, componentSpec) {
  ...
  // Container ID file.
  // @todo: bcauldwell - I think we can do away with this and just use
  // the container name as a container ID.
...
var loadPlugins = exports.loadPlugins = function(app, callback) {
  ...
  return Promise.map(app.plugins, kbox.require)
  .all()
  // Return.
  .nodeify(callback);
...
var installComponent = function(app, component, callback) {
  ...
  // Build image.
  .then(function() {
    return engine.buildAsync(component.image);
  })
  // Create container.
  .then(function() {
    return engine.createAsync(component.installOptions);
  })
  // Write cid file.
...
var startComponent = function(app, component, callback) {
  ...
  // Start component.
  .then(function() {
    return engine.startAsync(component.containerId, component.opts);
  })
  // Emit post event.
  .then(function() {
...
var stopComponent = function(component, callback) {
  ...
  // Stop container.
  .then(function() {
    return engine.stopAsync(component.containerId);
  })
  // Emit post event.
  .then(function() {
...
var stop = exports.stop = function(app, callback) {
  ...
  .then(function() {
    return Promise.resolve(getComponents(app))
    .map(stopComponent, {concurrency: 1})
    .all();
  })
  // Emit post event.
  .then(function() {
...
var uninstallComponent = function(component, callback) {
  ...
  // Uninstall container.
  .then(function() {
    return engine.removeAsync(component.containerId);
  })
...
var uninstall = exports.uninstall = function(app, callback) {
  ...
  // Stop components.
  // @todo: bcauldwell - We need to revisit what to do when uninstalling a
  //   container throws an error.
  .then(function() {
    return Promise.resolve(getComponents(app))
    .map(uninstallComponent, {concurrency: 1})
    .all();
  })
  // Emit post event.
  .then(function() {
