  ...
  .then(function(isUp) {
    ...
    if (isUp) {
      ...
      .filter(function(container) {
        return engine.isRunning(container.id);
      })
...
var stop = exports.stop = function(app, callback) {
  ...
  // Emit pre event.
  .then(function() {
    kbox.status.update('Running pre stop tasks.');
    return core.events.emit('pre-app-stop', app);
  })
  // Stop components.
  .then(function() {
    kbox.status.update('Stopping containers.');
    return engine.stop(getCompose(app));
  })
  // Emit post event.
  .then(function() {
    kbox.status.update('Running post stop tasks.');
...
exports.destroy = function(app, callback) {
  ...
  // Emit pre event.
  .then(function() {
    kbox.status.update('Running pre destroy tasks.');
  ...
  // Uninstall app.
  .then(function() {
    kbox.status.update('Uninstalling.');
  ...
  // Emit post event.
  .then(function() {
    kbox.status.update('Running post destroy tasks.');
