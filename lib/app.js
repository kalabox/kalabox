...
var createComponent = function(app, name, componentSpec) {
  ...
  component.name = name;
  component.hostname = (name === 'web') ? app.domain : [name, app.domain].join('.');
  ...
  core.deps.call(function(events) {
    services.verify(function(err) {
      if (err) {
        ...
        // @todo: this definitely shouldnt go here. this eventually needs to
        // someplace that is b2d specific and has the app context
        var cRoot = core.deps.lookup('config').codeRoot;
        if (!fs.existsSync(path.join(cRoot, app.name))) {
          mkdirp.sync(path.join(cRoot, app.name));
        }
