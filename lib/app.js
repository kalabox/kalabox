...
var App = function(appname) {
  ...
  this.domain = appname + '.' + config.domain;
  ...
  _.map(this.config.components, function(component, key) {
    ...
    component.hostname = component.key + '.' + _this.domain;
  ...
  var createContainer = function(component, done) {
    ...
    docker.createContainer({
      ...
      Dns: ['8.8.8.8', '8.8.4.4'],
      Env: ['APPNAME='+_this.appname, 'APPDOMAIN='+_this.domain]
  ...
  this.start = function(done) {
    ...
    async.mapSeries(components, startContainer, function(err, result) {
      ...
      /*
      async.mapSeries(components, setProxy, function(err, result) {
        console.log('done with setProxy');
      });
      */
    });
  };
