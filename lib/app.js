...
var createComponent = function(app, name, compose) {
  ...
  // See if the user has set their own compose containername
  var containerName;
  // jscs:disable
  /* jshint ignore:start */
  if (compose.container_name && !_.includes(compose.container_name, '$')) {
    containerName = component.container_name;
  }
  // jscs:enable
  /* jshint ignore:end */
  ...
  // Container name.
  component.containerName = containerName || ['kb', app.name, name].join('_');
  // Container ID.
...
var create = exports.create = function(name, config, callback) {
  ...
  .then(function(provider) {
    ...
    // Code dir
    app.codeDir = config.codeDir;
    // Code root
    app.codeRoot = config.codeRoot;
    // Container webroot
...
var uninstall = exports.uninstall = function(app, callback) {
  ...
  // Kill components.
  // NOTE: we are telling this to not remove volumes which is the default
  // behavior
  .then(function() {
    var killComponents = _.map(getComponents(app), function(component) {
...
var start = exports.start = function(app, callback) {
  ...
  // Start before containers, inspect and
  .then(function() {
  ...
  .then(function() {
    ...
    // Now set cids for each component
    // this is helpful if the user is not defining their own container
    // name
    .then(function() {
      return getComponents(app);
    })
    .each(function(component) {
      return engine.inspect({files: app.composeCore, service: component.name})
      .then(function(data) {
        component.cid = data.Id;
        component.containerId = _.trimLeft(data.Name, '/');
        component.containerName = component.containerId;
        var newComponent = createComponent(app, component.name, component);
        app = _.extend(app, {components: [newComponent]});
      });
    });
  })
